...
{
//	int stu_size = stu.stu_tag.size();
//	int depart_size = depart.dep_tag.size();
	int n=0;
...
//	cout << dep.num_mem << endl;
	for(i = 0; i < dep.num_mem-1 ; i++)
	{
		for(j = 0; j < dep.num_mem-1-i; j++)
		{
			...
			{
//				cout << dep.list[j].csm << " " << dep.list[j+1].csm << endl;
				app = dep.list[j];
...
/*
	for(i = 0;i<dep.num_mem;i++)
	{
		cout << dep.list[i].csm << " " << dep.list[i].ID << endl;
	}
*/
...
/*	for (vector<string>::iterator it1 = dep.apply.begin(); it1 != dep.apply.end(); ++it1)    
   {
		cout << *it1 << endl;
    }
*/
/*	for(int i=0;i<Numstu;i++)
	{
		for (vector<string>::iterator it1 = stu[i].dep_join.begin(); it1 != stu[i].dep_join.end(); ++it1)    
		{
			cout << *it1 ;
		}
		cout << endl;
...
int main()
{
	...
	ifstream in;
	in.open("create.json");
	//	ifstream in("create.json", ios::binary);
...
//		cout << depart[j].depID << " " << depart[j].need << endl;
	}
...
//		student_aspirations= cJSON_GetObjectItem(tstu,"student_aspirations");
//		int size4 = cJSON_GetArraySize(student_aspirations);
//		cout << size4 << endl;
/*
//		cJSON* taspi = NULL;
//		cJSON* ta = NULL;
		...
		{	
//			int c=0;
			cJSON* taspi = cJSON_GetArrayItem(student_aspirations,i);
			stu[j].aspi.push_back(taspi->valuestring);
...
//		cout << stu[j].stuID << " " << stu[j].gpa << endl;
